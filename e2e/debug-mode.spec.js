/**
 * Guards the environment every PHP-diagnostics assertion in this suite depends on.
 *
 * Those assertions work by reading PHP diagnostics out of the rendered document.
 * That only happens when both `WP_DEBUG` and `WP_DEBUG_DISPLAY` are enabled:
 * `wp_debug_mode()` leaves `display_errors` untouched when `WP_DEBUG` is false,
 * and then no diagnostic of any severity -- not even a fatal -- reaches the page,
 * so those assertions pass unconditionally.
 *
 ...
 * `WP_DEBUG: true` is not enough on its own. This test fails loudly if that
...
test.describe('PHP diagnostics are visible in the test environment', () => {
	test('WP_DEBUG and WP_DEBUG_DISPLAY are enabled', async ({ page }) => {
		...
		// Anchored on purpose. This environment is en_US, but were a Dutch locale
		// ever installed the values become Ingeschakeld/Uitgeschakeld -- and
		// "Uitgeschakeld" contains "geschakeld", so an unanchored match would pass
		// with WP_DEBUG switched off. toHaveText anchors the whole string.
